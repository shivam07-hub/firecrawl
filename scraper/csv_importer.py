...
_CITY_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"new delhi|new_delhi",  "New Delhi"),
    (r"\bdelhi\b",            "Delhi"),
    (r"gurgaon|gurugram",     "Gurgaon"),
    (r"noida",                "Noida"),
    (r"kolkata|calcutta",     "Kolkata"),
    (r"ahmedabad",            "Ahmedabad"),
    (r"kochi|cochin",         "Kochi"),
    (r"coimbatore",           "Coimbatore"),
    (r"jaipur",               "Jaipur"),
    (r"chandigarh",           "Chandigarh"),
    (r"bhubaneswar",          "Bhubaneswar"),
    (r"indore",               "Indore"),
    (r"nagpur",               "Nagpur"),
    (r"thiruvananthapuram|trivandrum", "Thiruvananthapuram"),
...
def _upsert_jobs(sb: Client, jobs: list[dict], batch_date: int | None) -> int:
    rows = []
    for job in jobs:
        ...
        row["location_city"]  = _extract_city(job.get("location") or "India")
...
def import_file(
    ...
    if not dry_run:
        jobs_written = _upsert_jobs(sb, jobs, batch_date)
    else:
...
def main() -> None:
    ...
    total_jobs = total_skill_rows = total_drift = 0
    t0 = time.time()

    for json_path in json_files:
        result = import_file(sb, json_path, skill_id_map, drift_counter, args.dry_run, run_id)
        if "error" in result:
