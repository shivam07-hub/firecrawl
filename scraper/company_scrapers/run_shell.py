#!/usr/bin/env python3
"""Standalone runner for Shell India IT Job Scraper.
Uses the Workday REST API (shell.wd3.myworkdayjobs.com).
Filters: locationCountry=India + jobFamilyGroup=Information Technology (IT).

API verified 2026-04-02 against shell.wd3.myworkdayjobs.com/shellcareers:
  POST https://shell.wd3.myworkdayjobs.com/wday/cxs/shell/shellcareers/jobs
  Body: {"appliedFacets": {"locationCountry": [INDIA_ID], "jobFamilyGroup": [IT_ID]},
         "limit": 20, "offset": N, "searchText": ""}

Facet IDs (verified 2026-04-02):
  India country:          c4f78be1a8f14da0ab49ce1162348a5e
  Information Technology: a87fe1bd64b8016d9c737d3fa72c300c  (~18 India IT jobs)
"""
...
BASE_URL    = "https://shell.wd3.myworkdayjobs.com"
API_URL     = f"{BASE_URL}/wday/cxs/shell/shellcareers/jobs"
CAREER_SITE = "shellcareers"

INDIA_COUNTRY_ID = "c4f78be1a8f14da0ab49ce1162348a5e"
IT_GROUP_ID      = "a87fe1bd64b8016d9c737d3fa72c300c"

MAX_JOBS        = 500
LIMIT_PER_PAGE  = 20

print("=" * 60)
print("SHELL INDIA IT JOB SCRAPER")
print("ATS: Workday (shell.wd3.myworkdayjobs.com/shellcareers)")
print("Filters: India + Information Technology (IT)")
print("=" * 60)
...
while offset < MAX_JOBS:
    ...
    try:
        ...
        for jp in postings:
            ...
            if external_path:
                try:
                    detail_url = f"{BASE_URL}/wday/cxs/shell/{CAREER_SITE}/{external_path}"
...
print(f"\n  Total Shell India+IT jobs: {len(jobs)}")

df = save_results(jobs, "Shell", OUTPUT_DIR)
if df is not None:
